  ...
  return (
    <div className="content-wrap">
      ...
      <Panel title="Planes de membresía" sub="precios y estado"
             action={<Btn kind="primary" size="sm" leading={I.plus}>Nuevo plan</Btn>}
             bodyPad={false}>
        <table className="tbl">
          <thead><tr><th>Plan</th><th>Duración</th><th className="num">Precio</th><th className="num">Activos</th><th>Estado</th></tr></thead>
          <tbody>
            {MEMBERSHIP_PLANS.map(p => (
              <tr key={p.id} className="clickable">
                <td className="cell-main">{p.n}</td>
                <td style={{ color: "var(--ink-2)" }}>{p.dur}</td>
                <td className="num" style={{ font: "700 14px var(--font-display)" }}>S/ {p.price}</td>
                <td className="num">{p.activos}</td>
                <td>{p.on ? <Badge kind="ok" dot>Disponible</Badge> : <Badge dot>Oculto</Badge>}</td>
              </tr>
            ))}
          </tbody>
        </table>

...
describe('RateLimitingGuard', () => {
  ...
  it('debe permitir peticiones que no son del endpoint /auth/login', async () => {
    const context = createMockContext('/api/v1/members', 'GET', {}, '127.0.0.1');
  ...
  it('debe permitir peticiones de login si el IP y el email no están bloqueados', async () => {
    ...
    const context = createMockContext('/api/v1/auth/login', 'POST', { email: 'test@gym.com' }, '192.168.1.10');
    const result = await guard.canActivate(context);

    expect(result).toBe(true);
    expect(redisService.exists).toHaveBeenCalledWith('rate:block:ip:192.168.1.10');
    expect(redisService.exists).toHaveBeenCalledWith('rate:block:email:test@gym.com');
  });
  ...
  it('debe lanzar HttpException 429 si el email del usuario está bloqueado', async () => {
    ...
    const context = createMockContext('/api/v1/auth/login', 'POST', { email: 'blocked@gym.com' }, '192.168.1.10');
  ...
  it('debe resetear contadores en Redis tras un login exitoso (status 200/201)', async () => {
    ...
    const context = createMockContext('/api/v1/auth/login', 'POST', { email: 'success@gym.com' }, '192.168.1.10');
    ...
    expect(redisService.del).toHaveBeenCalledWith('rate:fail:email:success@gym.com');
  });
  ...
  it('debe incrementar contadores y bloquear si se alcanzan los 5 intentos fallidos (status 401/403)', async () => {
    ...
    const context = createMockContext('/api/v1/auth/login', 'POST', { email: 'fail@gym.com' }, '192.168.1.10');
    const res = context.switchToHttp().getResponse() as any;
    ...
    // Debió incrementar
    expect(redisService.incrWithTtl).toHaveBeenCalledWith('rate:fail:ip:192.168.1.10', 900);
    expect(redisService.incrWithTtl).toHaveBeenCalledWith('rate:fail:email:fail@gym.com', 900);

    // Debió crear llaves de bloqueo por alcanzar 5
    expect(redisService.set).toHaveBeenCalledWith('rate:block:ip:192.168.1.10', '1', 900);
    expect(redisService.set).toHaveBeenCalledWith('rate:block:email:fail@gym.com', '1', 900);
  });
});
